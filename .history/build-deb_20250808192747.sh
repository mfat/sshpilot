...
if ! command -v dpkg-buildpackage >/dev/null 2>&1; then
  ...
  sudo apt update
  sudo apt install -y devscripts build-essential debhelper dh-python python3-all python3-setuptools
fi
...
# Update debian/changelog version from package __version__
VERSION=$(python3 -c "import pathlib, re; t=pathlib.Path('sshpilot/__init__.py').read_text(); print(re.search(r'__version__\s*=\s*\"([^\"]+)\"', t).group(1))")
