...
group row opens the whole group in split view (which confirms itself when the
group is large — see ``test_split_view_open_confirmation``).
...
def make_window(monkeypatch, row):
    ...
    window.on_open_group_in_split_view_action = lambda *_a: window.opened_splits.append(
...
def test_middle_click_on_a_connection_row_opens_a_tab(monkeypatch):
    ...
    assert window.opened_splits == []


def test_middle_click_on_a_group_row_opens_split_view(monkeypatch):
    ...
    assert window.opened_splits == [row]
