...
def test_startup_vault_unlock_waits_for_client_selection_then_runs_once(monkeypatch):
    """Regression: unlock_at_startup must not fire while the daemon client
    selection is still pending (``secrets_controller`` is None on the first
    main-loop iteration), and must fire exactly once after it settles.

    A single idle_add used to call unlock_at_startup before the async client
    selection populated the controller, so its first ``controller is None`` check
    silently no-opped and the startup vault-unlock prompt never appeared.
    """
    import pytest

    pytest.importorskip("gi")
    from sshpilot import main as main_module
    from sshpilot import secret_unlock_dialog

    unlock_calls = []

    def fake_unlock_at_startup(window):
        unlock_calls.append(window)
        return False
    ...
    monkeypatch.setattr(main_module.GLib, "timeout_add", fake_timeout_add)
    monkeypatch.setattr(
        secret_unlock_dialog, "unlock_at_startup", fake_unlock_at_startup
    )

    app = main_module.SshPilotApplication.__new__(main_module.SshPilotApplication)
    app.window = types.SimpleNamespace(_api_client_selection_pending=True)

    app._schedule_startup_vault_unlock()

    # While selection is pending, no unlock must have happened yet.
    assert unlock_calls == []
    assert scheduled, "expected the method to poll via timeout_add"

    # Selection settles; drive the pending poll callbacks to completion.
    app.window._api_client_selection_pending = False
    while scheduled:
        scheduled.pop(0)()

    assert unlock_calls == [app.window]


def test_startup_vault_unlock_runs_immediately_when_not_pending(monkeypatch):
    """When the client selection has already settled, unlock_at_startup is
    invoked directly (no polling) on the startup call.
    """
    import pytest
    ...
    app = main_module.SshPilotApplication.__new__(main_module.SshPilotApplication)
    app.window = types.SimpleNamespace(_api_client_selection_pending=False)
...
def test_startup_vault_unlock_no_window_is_safe(monkeypatch):
    """Without a window and with selection still pending, the method must not
    schedule anything or touch the dialog; it bails out cleanly.
    """
    import pytest
    ...
    # Exhaust the polling budget without a window ever appearing.
    app._schedule_startup_vault_unlock()
    while scheduled:
        scheduled.pop(0)()
