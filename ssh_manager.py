#!/usr/bin/env python3
"""
SSH Manager - A GTK-based SSH client with terminal integration
"""
...
class SSHManager:
    ...
    def on_activate(self, app):
        ...
        # Connection list store and view using modern ListView
        print("Creating ListStore...")
        self.connection_list = Gtk.ListStore(str, str, bool)  # name, hostname, connected
        
        # Create ListView with factory
        print("Creating ListView factory...")
        factory = Gtk.SignalListItemFactory()
        factory.connect('setup', self.on_list_item_setup)
        factory.connect('bind', self.on_list_item_bind)
        
        # Create selection model first
        print("Creating selection model...")
        self.selection_model = Gtk.SingleSelection(model=self.connection_list)
    ...
    def on_list_item_setup(self, factory, list_item):
        ...
        list_item.set_data("host_label", host_label)
    
    def on_list_item_bind(self, factory, list_item):
        """Bind data to list item"""
        item = list_item.get_item()
        if item:
            name_label = list_item.get_data("name_label")
            host_label = list_item.get_data("host_label")
            
            name_label.set_text(item[0])  # name
            host_label.set_text(item[1])  # hostname
    ...
    def update_button_states(self):
        ...
        selected = self.selection_model.get_selected_item()
        if selected:
            hostname = selected[1]
            connected = selected[2]
    ...
    def refresh_connection_list(self):
        ...
        self.connection_list.remove_all()
        for conn in self.connections:
            connected = conn.hostname in self.connected_connections
            self.connection_list.append([conn.name, conn.hostname, connected])
    ...
    def on_connect(self, button):
        ...
        if selected:
            hostname = selected[1]
            conn = next((c for c in self.connections if c.hostname == hostname), None)
            
            if conn and hostname not in self.connected_connections:
                # Check if tab already exists
                existing_tab = self.find_tab_by_hostname(hostname)
                if existing_tab:
