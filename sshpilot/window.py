...
class ConnectionRow(Adw.ActionRow):
    ...
    def __init__(self, connection: Connection):
        ...
        # Create port forwarding indicators (L/R/D)
        ...
        # Add indicators to the row
        self.add_suffix(self.indicator_box)
        self.add_suffix(self.status_icon)
        
        # Set up gestures for single and double click
    ...
    def set_selected(self, selected: bool):
        ...
        if selected:
            ...
            self.add_css_class('selection-highlight')
        else:
            self.remove_css_class('selected')
            self.remove_css_class('selection-highlight')
    ...
    def _install_pf_css():
        try:
            ...
            .selection-highlight {
                background-color: @accent_bg_color; /* Use accent color for highlight */
            ...
            .selection-highlight:hover {
                background-color: shade(@accent_bg_color, 0.9); /* Slightly darker on hover */
            }
            """
    ...
    def _apply_host_label_text(self):
        ...
        if hide:
            ...
            self.set_subtitle(f"{self.connection.username}@{self.connection.host}")
    ...
    def update_display(self):
        ...
        # Update the title and subtitle with current connection data
        if hasattr(self.connection, 'nickname'):
            self.set_title(self.connection.nickname)
        
        if hasattr(self.connection, 'username') and hasattr(self.connection, 'host'):
            port_text = f":{self.connection.port}" if hasattr(self.connection, 'port') and self.connection.port != 22 else ""
            self.set_subtitle(f"{self.connection.username}@{self.connection.host}{port_text}")
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def setup_ui(self):
        ...
        logger.info("Loading UI from resources...")
        
        # Try to load from resources first
        try:
            self.builder = Gtk.Builder()
            self.builder.add_from_resource("/io/github/mfat/sshpilot/ui/main_window.ui")
            logger.info("Successfully loaded UI from resources")
        except Exception as e:
            logger.warning(f"Failed to load from resources: {e}")
            # Fallback to local file
            logger.info("Loading UI from local file...")
            try:
                self.builder = Gtk.Builder()
                self.builder.add_from_file("sshpilot/ui/main_window.ui")
                logger.info("Successfully loaded UI from local file")
            except Exception as e2:
                logger.error(f"Failed to load UI file: {e2}")
        ...
        try:
            ...
            self.connection_list = self.builder.get_object("connection_list")
            ...
            self.tab_view = self.builder.get_object("tab_view")
            self.tab_bar = self.builder.get_object("tab_bar")
        except Exception as e:
            logger.error(f"Failed to get UI objects: {e}")
            self._create_fallback_ui()
            return
    ...
    def setup_content_area(self):
        """Set up the main content area with tab view"""
        # The tab view and tab bar are now loaded from the UI file
        ...
        if hasattr(self, 'tab_bar') and self.tab_bar and hasattr(self, 'tab_view') and self.tab_view:
    ...
    def _create_fallback_ui(self):
        ...
        logger.warning("Creating fallback UI")
        ...
        header_bar.set_title_widget(Gtk.Label(label="sshPilot"))
        ...
        sidebar.set_title("Connections")
        
        # Create connection list
        self.connection_list = Adw.PreferencesGroup()
        self.connection_list.set_title("Connections")
        sidebar.add(self.connection_list)
        ...
        self.split_view.set_overlay(self.tab_bar)
    ...
    def setup_connections(self):
        ...
        # For AdwPreferencesGroup, we can't easily select the first row
        # Just ensure the connection list has focus
        if connections:
            GLib.idle_add(self._focus_connection_list_first_row)
    ...
    def show_tab_view(self):
        ...
        # The tab view is always visible now since it's directly in the UI
        # Don't apply terminal-bg to the tab view itself
        pass
    ...
    def connect_to_host(self, connection: Connection, force_new: bool = False):
        ...
        try:
            ...
            logger.info(f"Terminal widget created successfully: {terminal}")
        except Exception as e:
            logger.error(f"Failed to create terminal widget: {e}")
            import traceback
        ...
        try:
            ...
            logger.info(f"Terminal added to tab view successfully")
            
            # Ensure the terminal widget is properly configured for expansion
            terminal.set_hexpand(True)
            terminal.set_vexpand(True)
            terminal.set_size_request(600, 400)
            
            # Ensure the tab view is properly configured
            self.tab_view.set_hexpand(True)
            self.tab_view.set_vexpand(True)
            
        except Exception as e:
            logger.error(f"Failed to add terminal to tab view: {e}")
            import traceback
            logger.error(f"Traceback: {traceback.format_exc()}")
            return
        
        # Debug: Check if terminal is properly added
        logger.info(f"Added terminal to tab view: {terminal}")
        logger.info(f"Tab page created: {page}")
        logger.info(f"Terminal widget visible: {terminal.get_visible()}")
        logger.info(f"Terminal widget mapped: {terminal.get_mapped()}")
        logger.info(f"Terminal widget realized: {terminal.get_realized()}")
        logger.info(f"Terminal widget allocated: {terminal.get_allocated_width()}x{terminal.get_allocated_height()}")
        ...
        # Simple terminal visibility check
        def _check_terminal_visibility():
            try:
                logger.info(f"Terminal widget allocated: {terminal.get_allocated_width()}x{terminal.get_allocated_height()}")
                logger.info(f"Terminal widget visible: {terminal.get_visible()}")
                logger.info(f"Terminal widget mapped: {terminal.get_mapped()}")
            except Exception as e:
                logger.error(f"Error checking terminal visibility: {e}")
        
        # Schedule the visibility check
        GLib.timeout_add(500, _check_terminal_visibility)
        
        # Debug: Check tab view state
        logger.info(f"Tab view pages count: {self.tab_view.get_n_pages()}")
        logger.info(f"Selected page: {self.tab_view.get_selected_page()}")
        logger.info(f"Tab view visible: {self.tab_view.get_visible()}")
        logger.info(f"Tab view mapped: {self.tab_view.get_mapped()}")
        logger.info(f"Tab view allocated: {self.tab_view.get_allocated_width()}x{self.tab_view.get_allocated_height()}")
        
        # Check if the tab view is properly showing the terminal
        def _check_tab_view_display():
            try:
                logger.info(f"Tab view pages count after delay: {self.tab_view.get_n_pages()}")
                selected_page = self.tab_view.get_selected_page()
                logger.info(f"Selected page after delay: {selected_page}")
                if selected_page:
                    logger.info(f"Selected page title: {selected_page.get_title()}")
                    logger.info(f"Selected page child: {selected_page.get_child()}")
                    logger.info(f"Selected page child visible: {selected_page.get_child().get_visible() if selected_page.get_child() else 'No child'}")
                    logger.info(f"Selected page child allocated: {selected_page.get_child().get_allocated_width()}x{selected_page.get_child().get_allocated_height() if selected_page.get_child() else 'No child'}")
            except Exception as e:
                logger.error(f"Error checking tab view display: {e}")
        
        # Schedule the check to run after a short delay
        GLib.timeout_add(1000, _check_tab_view_display)
    ...
    def on_connection_activated(self, action_row):
        """Handle connection activation (double-click on AdwActionRow)"""
        logger.info(f"Connection activated: {action_row}")
        if hasattr(action_row, 'connection') and action_row.connection:
            logger.info(f"Connection activated: {action_row.connection.nickname}")
            # Open connection
            self._cycle_connection_tabs_or_open(action_row.connection)
        else:
            logger.warning("Connection activated but no connection found in action_row")
    
    def on_connection_selected(self, action_row):
        """Handle connection selection (single-click on AdwActionRow)"""
        logger.info(f"Connection selected: {action_row}")
        if hasattr(action_row, 'connection') and action_row.connection:
            logger.info(f"Connection selected: {action_row.connection.nickname}")
            # Clear previous selection
            if hasattr(self, 'selected_connection') and self.selected_connection and self.selected_connection in self.connection_rows:
                previous_row = self.connection_rows[self.selected_connection]
                previous_row.set_selected(False)

            # Update selection
            self.selected_connection = action_row.connection
            action_row.set_selected(True)
            self._update_button_sensitivity()
        else:
            logger.warning("Connection selected but no connection found in action_row")
        
    def on_activate_connection(self, action, param):
        """Handle the activate-connection action"""
        if self.selected_connection:
            self._cycle_connection_tabs_or_open(self.selected_connection)
    ...
    def _update_button_sensitivity(self):
        ...
        has_selection = self.selected_connection is not None
    ...
    def on_add_connection_clicked(self, button):
        ...
        logger.info("Add connection button clicked!")
        self.show_connection_dialog()
    
    def on_menu_button_clicked(self, button):
        """Handle menu button click"""
        self._show_application_menu()
    ...
    def _show_application_menu(self):
        ...
        try:
            ...
            popover.set_pointing_to(self.menu_button)
            ...
            logger.error(f"Failed to show application menu: {e}")

    def on_edit_connection_clicked(self, button):
        """Handle edit connection button click"""
        if self.selected_connection:
            self.show_connection_dialog(self.selected_connection)
    ...
    def on_open_new_connection_action(self, action, param=None):
        ...
        try:
            ...
            if connection is None:
                # Fallback to selected connection if any
                connection = self.selected_connection
            if connection is None:
                return
