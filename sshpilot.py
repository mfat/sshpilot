#!/usr/bin/env python3
"""
SSH Pilot - Completely clean implementation
"""
...
from gi.repository import Gtk, Vte, GLib, Gdk, Gio, Pango, Secret
import json
import os
import threading
...
class SecurePasswordManager:
    """Secure password storage using GNOME Keyring/libsecret"""
    
    def __init__(self):
        self.schema = Secret.Schema.new(
            "org.sshpilot.password",
            Secret.SchemaFlags.NONE,
            {
                "hostname": Secret.SchemaAttributeType.STRING,
                "username": Secret.SchemaAttributeType.STRING,
                "port": Secret.SchemaAttributeType.STRING,
                "type": Secret.SchemaAttributeType.STRING,  # "password" or "key_passphrase"
            }
        )
    
    def store_password(self, hostname, username, port, password, password_type="password"):
        """Store a password securely in the keyring"""
        if not password:
            return False
            
        try:
            label = f"SSHPilot - {username}@{hostname}:{port} ({password_type})"
            attributes = {
                "hostname": hostname,
                "username": username, 
                "port": str(port),
            ...
            print(f"=== SECURE: Stored {password_type} for {username}@{hostname}:{port} ===")
            return True
        except Exception as e:
            print(f"=== SECURE: Error storing {password_type}: {e} ===")
            return False
    
    def retrieve_password(self, hostname, username, port, password_type="password"):
        ...
        try:
            ...
            if password:
                print(f"=== SECURE: Retrieved {password_type} for {username}@{hostname}:{port} ===")
            else:
                print(f"=== SECURE: No {password_type} found for {username}@{hostname}:{port} ===")
            
            return password
        except Exception as e:
            print(f"=== SECURE: Error retrieving {password_type}: {e} ===")
            return None
    
    def delete_password(self, hostname, username, port, password_type="password"):
        ...
        try:
            ...
            if result:
                print(f"=== SECURE: Deleted {password_type} for {username}@{hostname}:{port} ===")
            
            return result
        except Exception as e:
            print(f"=== SECURE: Error deleting {password_type}: {e} ===")
            return False
    
    def delete_all_passwords(self, hostname, username, port):
        """Delete all passwords (both password and key_passphrase) for a connection"""
        results = []
        results.append(self.delete_password(hostname, username, port, "password"))
        results.append(self.delete_password(hostname, username, port, "key_passphrase"))
        return any(results)
...
class FreshSSHManager:
    def __init__(self):
        ...
        self.config_file = os.path.expanduser("~/.sshpilot_config.json")
        
        # Initialize secure password manager
        self.password_manager = SecurePasswordManager()
        print("=== FRESH: Secure password manager initialized ===")
    ...
    def load_connections(self):
        """Load saved connections from config file (passwords retrieved from keyring)"""
            ...
            try:
                with open(self.config_file, 'r') as f:
                    ...
                    for conn_data in data.get('connections', []):
                        ...
                        # Retrieve passwords securely from keyring
                        password = self.password_manager.retrieve_password(
                            hostname, username, port, "password"
                        )
                        key_passphrase = self.password_manager.retrieve_password(
                            hostname, username, port, "key_passphrase"
                        )
                        
                        # Handle legacy configs that still have plain text passwords
                        # (these will be migrated to keyring on next save)
                        if not password and conn_data.get('password'):
                            password = conn_data.get('password')
                            print(f"=== FRESH: Migrating legacy password for {username}@{hostname} ===")
                        
                        if not key_passphrase and conn_data.get('key_passphrase'):
                            key_passphrase = conn_data.get('key_passphrase')
                            print(f"=== FRESH: Migrating legacy key passphrase for {username}@{hostname} ===")
    ...
    def save_connections(self):
        """Save connections to config file (passwords stored securely in keyring)"""
        ...
        for conn in self.connections:
            ...
            # Store SSH key path in config (not sensitive)
            if conn.key_path:
                conn_data['key_path'] = conn.key_path
            
            # Store passwords securely in keyring (NOT in config file)
            if conn.password:
                self.password_manager.store_password(
                    conn.hostname, conn.username, conn.port, 
    ...
    def on_delete_confirmation_response(self, dialog, response, conn):
        ...
        if response == Gtk.ResponseType.YES:
            ...
            # Clean up stored passwords from keyring
            self.password_manager.delete_all_passwords(conn.hostname, conn.username, conn.port)
...
• Ctrl+L - Switch between connections
