#!/usr/bin/env python3
"""
Comprehensive test suite for SFTP file manager operations.
Uses AsyncSFTPManager directly to test all scenarios without GUI.

Usage:
    python3 test_sftp_comprehensive.py [--connection NICKNAME] [--remote-dir PATH] [--verbose]
"""
...
class SFTPTester:
    """Comprehensive test suite for SFTP operations"""
    ...
    def setup(self):
        ...
        # Initialize GTK (required for GObject signals)
        ...
        try:
            ...
            # Use key-based authentication (password=None means use keys)
            password = None
            auth_method = getattr(self.connection, "auth_method", 0) if self.connection else 0
            logger.info(f"Connection auth_method: {auth_method} (1=password, 0=key)")
            logger.info("Using key-based authentication for testing")
            
            # Temporarily override auth_method to allow key-based auth for testing
            original_auth_method = None
            if self.connection:
                original_auth_method = getattr(self.connection, "auth_method", None)
                # Force key-based auth for testing
            ...
            # Keep auth_method overridden until after connection is established
            # We'll restore it in cleanup()
            self._original_auth_method = original_auth_method
            
            # Set up progress tracking
            def on_progress(manager, progress, message):
                self.progress_updates.append((time.time(), progress, message))
                logger.debug(f"Progress: {progress*100:.1f}% - {message}")
            
            self.manager.connect("progress", on_progress)
            
            # Set up authentication handler (for key passphrases if needed)
            def on_auth_required(manager, prompt):
                logger.warning(f"Authentication required: {prompt}")
                # For key-based auth, this might be a passphrase request
                # We'll let it fail gracefully if passphrase is needed
                logger.info("Key passphrase may be required - test will continue with available keys")
            ...
            self.manager.connect("connected", on_connected)
            self.manager.connect("connection-error", on_error)
            self.manager.connect("authentication-required", on_auth_required)
            
            while waited < max_wait:
                self.process_gtk_events()
                
                if connected or self.manager._sftp is not None:
            ...
            if self.manager._sftp is None:
                raise RuntimeError("Failed to connect to server within timeout")
            
        except Exception as e:
            logger.error(f"Failed to create SFTP manager: {e}", exc_info=True)
            raise
    ...
    def wait_for_operation(self, future: Future, timeout: int = 120) -> Tuple[bool, Optional[str]]:
    ...
    def test_single_file_upload(self, file_path: pathlib.Path):
        ...
        try:
            ...
            if success:
                # Verify file exists
                verify_future = self.manager.path_exists(remote_path)
                verify_success, _ = self.wait_for_operation(verify_future, timeout=10)
                exists = verify_future.result() if verify_success else False
                ...
                if exists:
                    ...
                    logger.error(f"✗ File uploaded but not found on server: {file_path.name}")
                    self.test_results.append(("single_file_upload", file_path.name, False, "File not found after upload"))
                    self.errors.append(f"File uploaded but not found: {file_path.name}")
            else:
    ...
    def test_multiple_files_upload(self, file_paths: List[pathlib.Path]):
        ...
        try:
            ...
            for file_path in file_paths:
                ...
                # Small delay between starting uploads to avoid overwhelming connection
                time.sleep(0.1)
            
            # Wait for all to complete with longer timeout for multiple files
            all_success = True
            errors = []
            for filename, future in futures:
                # Use longer timeout for multiple simultaneous operations
                success, error = self.wait_for_operation(future, timeout=180)
                if not success:
                    all_success = False
    ...
    def test_multiple_dirs_upload(self, dir_paths: List[pathlib.Path]):
        """Test uploading multiple directories simultaneously"""
    ...
    def test_download(self, remote_filename: str):
        ...
        try:
            ...
            if success and local_path.exists():
                # Verify content
                original_size = (self.local_test_dir / remote_filename).stat().st_size if (self.local_test_dir / remote_filename).exists() else 0
                downloaded_size = local_path.stat().st_size
                
                if downloaded_size > 0:
                    logger.info(f"✓ Download succeeded: {remote_filename} ({downloaded_size} bytes)")
                    logger.info(f"  Progress updates: {len(self.progress_updates)}")
                    self.test_results.append(("download", remote_filename, True, None))
                else:
                    logger.error(f"✗ Downloaded file is empty: {remote_filename}")
                    self.test_results.append(("download", remote_filename, False, "Downloaded file is empty"))
                    self.errors.append(f"Downloaded file is empty: {remote_filename}")
            else:
    ...
    def cleanup(self):
        ...
        # Restore original auth_method
        if hasattr(self, '_original_auth_method') and self._original_auth_method is not None and self.connection:
            self.connection.auth_method = self._original_auth_method
            logger.info(f"Restored auth_method to {self._original_auth_method}")
