...
DEFAULT_COMMANDS = [
    ...
    # Network
    {
        ...
        "description": "All interfaces and IPs",
        "tags": ["network"],
        "folder_id": "f-network",
        "is_favorite": True,
        "has_placeholders": False,
    ...
    # Processes
    {
        ...
        "description": "20 most CPU-hungry processes",
    ...
    },
    {
        ...
        "description": "One-shot top output",
        "tags": ["processes"],
        "folder_id": "f-procs",
        "is_favorite": False,
        "has_placeholders": False,
    ...
    },
    {
        ...
        "description": "Resource usage per container",
    ...
    },
    {
        ...
        "description": "Last 100 lines of service journal",
        "tags": ["systemd", "logs"],
        "folder_id": "f-systemd",
        "is_favorite": False,
        "has_placeholders": True,
    },
    {
        ...
        "description": "All failed systemd units",
        "tags": ["systemd"],
        "folder_id": "f-systemd",
        "is_favorite": False,
        "has_placeholders": False,
    ...
    },
    {
        ...
        "description": "Size of each log file/dir",
        "tags": ["disk", "logs"],
        "folder_id": "f-disk",
        "is_favorite": False,
        "has_placeholders": False,
    ...
    # Logs
    {
        ...
        "description": "Live syslog (Ctrl+C to stop)",
        "tags": ["logs"],
        "folder_id": "f-logs",
        "is_favorite": False,
        "has_placeholders": False,
    ...
    },
    {
        ...
        "description": "Live systemd journal",
        "tags": ["logs", "systemd"],
        "folder_id": "f-logs",
        "is_favorite": False,
        "has_placeholders": False,
    ...
    },
    {
        ...
        "description": "Last 50 error lines in syslog",
        "tags": ["logs"],
        "folder_id": "f-logs",
        "is_favorite": False,
        "has_placeholders": False,
    ...
    # Security
    {
        ...
        "name": "Recent logins",
        "command": "last -n 20",
        "description": "Last 20 login events",
        "tags": ["security"],
        "folder_id": "f-security",
        "is_favorite": False,
        "has_placeholders": False,
    },
    {
    ...
    },
    {
        ...
        "description": "Last 20 failed login attempts",
        "tags": ["security"],
        "folder_id": "f-security",
        "is_favorite": False,
        "has_placeholders": False,
                ...
                f.update(
                    {
                        ...
                        if k in f or k in ("name", "expanded", "order", "parent_id")
                    }
                )
                self._save()
                return
...
class FolderRow(Gtk.ListBoxRow):
    ...
    def _update_expand_icon(self) -> None:
        icon_name = (
            "pan-down-symbolic" if self._folder.get("expanded", True) else "pan-end-symbolic"
        )
...
class CommandEditDialog(Adw.Window):
    ...
    def __init__(
        self, parent: Gtk.Window, store: CommandBlockStore, cmd: dict | None = None
    ) -> None:
        super().__init__()
        self._store = store
        self._cmd = cmd
    ...
    def _build_ui(self) -> None:
        ...
        self._placeholder_row = Adw.SwitchRow(
            title=_("Has Placeholders"), subtitle=_("Use ${VAR} syntax in command")
        )
        if self._cmd:
    ...
    def _on_save(self, *_) -> None:
        ...
        folder_id = (
            self._folder_names_list[selected_idx]
            if selected_idx < len(self._folder_names_list)
            else None
        )
...
class CommandBlocksPanel(Gtk.Box):
    ...
    def _insert_folder_commands(self, folder_row: FolderRow, folder_id: str) -> None:
        ...
        while sibling is not None:
            if isinstance(sibling, FolderRow):
                break
            if isinstance(sibling, CommandRow) and sibling._cmd_data.get("folder_id") == folder_id:
                return
    ...
    def _remove_folder_commands(self, folder_row: FolderRow, folder_id: str) -> None:
        ...
        selected_in_folder = (
            isinstance(selected, CommandRow) and selected._cmd_data.get("folder_id") == folder_id
        )
    ...
    def _do_broadcast(self, command_text: str, cmd_id: str | None = None) -> None:
        ...
        targets = terminal_manager.broadcast_connection_ids()
        if not targets:
            self._show_toast(_("No SSH terminals open — connect to a server first"), timeout=3)
            return

        def accepted(summary):
            self._show_toast(
                _("Command queued for {} connections").format(len(summary.targets)), timeout=3
            )
            if cmd_id and summary.targets:
        ...
        terminal_manager.run_command_on_connections(
            ...
            on_error=lambda _error: self._show_toast(
                _("Broadcast command could not be started"), timeout=3
            ),
        )
    ...
    def show_command_picker_for_target(
        ...
        *connections* (a list of Connection objects, e.g. a sidebar
        ...
        def _on_activated(_lb, list_row):
            ...
            popover.popdown()
            if cmd is None:
                self._show_custom_command_dialog(
                    connection=connection, group=group, connections=connections
                )
            else:
                self._run_cmd_block_on_target(
                    cmd, connection=connection, group=group, connections=connections
                )
    ...
    def _run_cmd_block_on_target(
        self, cmd: dict, *, connection=None, group=None, connections=None
    ) -> None:
        if cmd.get("has_placeholders"):
            dlg = PlaceholderDialog(self.window, cmd)
            dlg.connect(
    ...
    def _show_run_on_host_picker(self, cmd: dict, anchor: Gtk.Widget) -> None:
        ...
        show_host_picker(
            self.window,
            anchor,
            lambda conn: self._run_command_on_connection(cmd, conn),
    ...
    def _feed_interactive_when_connected(
        ...
        if shell_ready is not None:
            terminal.feed_child_data_when_shell_ready(data)
            return
    ...
    def _show_command_context_menu(self, row: Gtk.ListBoxRow, cmd: dict) -> None:
        ...
        menu.add_section(
            ...
            menu.add_item(
                "edit-copy-symbolic", _("Duplicate"), lambda: self._duplicate_command(cmd)
            ),
            menu.add_item(
                "document-send-symbolic",
                _("Broadcast Command"),
                lambda: self._broadcast_command(cmd),
            ),
            menu.add_item(
                "computer-symbolic",
                _("Run on host…"),
                lambda: self._show_run_on_host_picker(cmd, row),
            ),
            menu.add_item(
                "folder-symbolic",
                _("Run for group…"),
                lambda: self._show_run_for_group_picker(cmd, row),
            ),
        )
