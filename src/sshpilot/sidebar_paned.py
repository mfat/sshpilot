...
``get_sidebar_width`` / ``get_resting_sidebar_width``
    The live width, and the width the sidebar would rest at (what the search
    popup uses for its panel width).
...
fraction-derived width from then on. The owner is told about it through
``on_user_resize`` so it can be persisted; the callback is debounced so a drag
writes the setting once. ``on_drag`` fires on every divider move while the
pointer is dragging (not on layout reflow), so the owner can pause the tips
banner for the duration of the gesture.

Icon-strip ("minimal") mode and divider-driven mode switches were removed; a
drag into the wall simply stops at the sidebar's content floor.
...
class _ClipStart(Gtk.Widget):
    ...
    with the divider, so what gets cut off is the left of the sidebar. A split
    view clips the other way. This bin lays the child out at its minimum from
    x=0 and hides the overflow.
...
class SidebarPaned(Gtk.Paned):
    ...
    def _free_position(self, width: int) -> int:
        """Divider position for a ``width``-wide split."""
    ...
    def _target_position(self, width: int) -> int:
        """Resting divider position for a ``width``-wide split."""
    ...
    def _on_position_notify(self, *_args) -> None:
        """Clamp the divider and remember a dragged width."""
        if self._applying:
            return
        width = self.get_width()
    ...
    def get_resting_sidebar_width(self) -> int:
        """Width the sidebar rests at for the current window size."""
