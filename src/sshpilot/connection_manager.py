...
from .ssh_config_formatter import (  # noqa: F401  (re-export)
    MANAGED_HOST_OPTIONS,
    SSH_UUID_MARKER,
...
class Connection:
    ...
    @property
    def id(self) -> str:
        """Concrete SSH Host alias as saved connection ID."""
        return self.nickname
...
class ConnectionManager(GObject.Object):
    ...
    def __init__(
        ...
        *,
        ...
        self._visible_connections_by_id: Dict[str, Connection] = (
    ...
    def sync_domain_from_live_connections(self, group_manager=None) -> None:
        ...
        for connection in list(self.connections):
            ...
            data.pop("uuid", None)
            data["id"] = getattr(connection, "id", "") or getattr(connection, "nickname", "")
            ...
            if gm is not None:
                primary = gm.connections.get(getattr(connection, "id", "") or getattr(connection, "nickname", ""))
                if primary:
    ...
    def _register_connection(self, connection: Connection) -> None:
        ...
        id_index = getattr(self, '_connections_by_id', None)
        if not isinstance(id_index, dict):
            id_index = {}
            self._connections_by_id = id_index
        connection._connection_manager = self
        if connection not in self.connections:
            self.connections.append(connection)
        id_index[connection.id] = connection
        id_index[connection.nickname] = connection
    ...
    def _serializable_connection_data(data: Dict[str, Any]) -> Dict[str, Any]:
        return {
            ...
            if not k.startswith('__') and k not in ('password', 'password_changed', 'uuid')
        }
    ...
    def _load_non_ssh_connections(
        ...
        for data in stored:
            ...
            try:
                ...
                repaired_data.pop('uuid', None)
                nickname = repaired_data.get('nickname') or repaired_data.get('id') or ''
                repaired_data['nickname'] = nickname
                repaired_data['id'] = nickname
    ...
    def _load_ssh_config_locked(
        ...
        *,
        ...
        prev_id_index = (
            getattr(
                self,
                '_visible_connections_by_id',
                getattr(self, '_connections_by_id', {}),
            )
            if not reuse_existing
            else getattr(self, '_connections_by_id', {})
        )
        ...
        try:
            if not os.path.exists(self.ssh_config_path):
                ...
                self._visible_connections_by_id = self._connections_by_id
            ...
            def _materialise(token: str, raw_cfg: Dict[str, Any], tokens: List[str], cfg_file: str):
                ...
                existing = existing_by_nickname.get(token)
                if existing:
            ...
            def _absorb_option_lines(raw_lines, config: Dict[str, Any]) -> None:
                ...
                for raw_line in raw_lines:
                    ...
                    if line.startswith('#'):
                        ...
                        # Ignore legacy ConnectionUUID comments and other comments.
                        continue
            ...
            self._connections_by_id = prev_id_index
            self._visible_connections = prev_connections
            self._visible_connections_by_id = prev_id_index
    ...
    def format_ssh_config_entry(self, data: Dict[str, Any]) -> str:
        """Format connection data as an SSH config entry (delegates to
        ssh_config_formatter; kept as a method for existing callers)."""
    ...
    def _merged_block_lines(self, old_block: Optional[HostBlock],
        ...
        """Surgical block rendering (delegates to ssh_config_formatter)."""
    ...
    def update_ssh_config_file(self, connection: Connection, new_data: Dict[str, Any], original_nickname: Optional[str] = None):
        ...
        if 'id' not in new_data:
            ...
            if connection_id:
                new_data = dict(new_data)
                new_data['id'] = connection_id
                new_data['nickname'] = new_data.get('nickname') or connection_id
        new_data = dict(new_data)
    ...
    def add_connection_from_data(self, data: Dict[str, Any]) -> Connection:
        ...
        # Canonical domain create (alias + field validation).
        try:
    ...
    def update_connection(
        ...
        *,
        ...
        try:
            ...
            new_data['id'] = new_data.get('nickname') or connection.nickname
            new_data['nickname'] = new_data.get('nickname') or connection.nickname
            ...
            try:
                ...
                payload['nickname'] = connection.nickname
        ...
        try:
            ...
            if getattr(connection, 'protocol', 'ssh') == 'ssh':
                ...
                if connection in self.connections:
                    self.connections.remove(connection)
                    getattr(self, '_connections_by_id', {}).pop(
                        connection.id,
                        None,
                    )
            else:
                ...
                if not self._persist_non_ssh_connections():
                    if idx >= 0:
                        ...
                        self._connections_by_id = getattr(
                            self,
                            '_connections_by_id',
                            {},
                        )
            ...
            try:
                ...
                if isinstance(meta_all, dict) and connection.id in meta_all:
                    del meta_all[connection.id]
            ...
            try:
                self.domain.delete_if_present(connection.id)
            except Exception:
    ...
    def find_connection_by_nickname(self, nickname: str) -> Optional[Connection]:
        """Find connection by Host alias / nickname / ID."""
        connections = (
        ...
        for connection in connections:
            if connection.nickname == nickname or connection.id == nickname:
                return connection
        index = getattr(self, '_connections_by_id', {})
        return index.get(nickname)

    def get_connection_by_id(self, connection_id: str) -> Optional[Connection]:
        """Look up a connection by SSH Host alias / id."""
        return self.find_connection_by_nickname(connection_id)

    def get_connection_by_uuid(self, connection_uuid: str) -> Optional[Connection]:
        """Deprecated alias for :meth:`get_connection_by_id`."""
        return self.get_connection_by_id(connection_uuid)
