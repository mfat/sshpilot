...
class ImagesTabMixin:
    def _build_images_section(self) -> Gtk.Widget:
        ...
        self._images_placeholder = self._make_loading_placeholder(_("Loading images…"))
    ...
    def _refresh_images(self) -> None:
        ...
        if client is None:
            return
        self._set_placeholder_loading(self._images_placeholder, _("Loading images…"))
    ...
    def _on_images(self, rows: Optional[List[dict]], err: Optional[Exception]) -> None:
        ...
        if not rows:
            self._set_placeholder_idle(self._images_placeholder, _("No images"))
            return
    ...
    def _pull_image(self) -> None:
        ...
        def on_ok(ref: str) -> None:
            ok = self._open_command_terminal(
                nick,
                client.pull_command(ref),
                title=_("Pull: {reference}").format(reference=ref),
            )
            if not ok:
                self._toast(_("Could not start pull"))

        prompt_text(
            self._window(),
            _("Pull image"),
            _("Image reference to pull (e.g. nginx:latest)."),
            "nginx:latest",
            _("Pull"),
            on_ok,
        )
    ...
    def _image_history(self, ref: str) -> None:
        ...
        def render(rows: List[dict]) -> str:
            ...
            return "\n\n".join(lines) or _("(no layers)")

        def done(rows: Optional[List[dict]], err: Optional[Exception]) -> None:
            if err is not None:
                self._toast(
                    _("History for {reference} failed: {detail}").format(
                        reference=ref,
                        detail=err,
                    )
                )
            ...
            TextViewDialog(
                self._window(),
                _("{reference} — history").format(reference=ref),
                render(rows or []),
            ).present()
    ...
    def _on_prune(self, res: Any, err: Optional[Exception]) -> None:
        if err is not None or (res is not None and getattr(res, "exit_code", 0) != 0):
            self._toast(
                _("Prune failed: {detail}").format(
                    detail=err or getattr(res, "stderr", ""),
                )
            )
            return
        ...
        if out:
            TextViewDialog(self._window(), _("Prune result"), out).present()
        else:
            self._toast(_("Prune complete"))
        self._refresh_images()
