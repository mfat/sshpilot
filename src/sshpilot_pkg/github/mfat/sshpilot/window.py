...
class ConnectionRow(Gtk.ListBoxRow):
    ...
    def update_display(self):
        ...
        self.host_label.set_text(f"{self.connection.username}@{self.connection.host}:{self.connection.port}")
        self.update_status()
...
class ConnectionDialog(Adw.Window):
    ...
    # Template widget bindings
    main_stack = Gtk.Template.Child()
    cancel_button = Gtk.Template.Child()
    save_button = Gtk.Template.Child()
    # Basic settings
    nickname_row = Gtk.Template.Child()
    host_row = Gtk.Template.Child()
    username_row = Gtk.Template.Child()
    port_row = Gtk.Template.Child()
    # Authentication
    auth_method_row = Gtk.Template.Child()
    keyfile_row = Gtk.Template.Child()
    key_passphrase_row = Gtk.Template.Child()
    password_row = Gtk.Template.Child()
    # Advanced
    x11_row = Gtk.Template.Child()
    # Port forwarding - Local
    local_forward_enabled = Gtk.Template.Child()
    local_listen_addr = Gtk.Template.Child()
    local_listen_port = Gtk.Template.Child()
    local_dest_addr = Gtk.Template.Child()
    local_dest_port = Gtk.Template.Child()
    # Port forwarding - Remote
    remote_forward_enabled = Gtk.Template.Child()
    remote_listen_addr = Gtk.Template.Child()
    remote_listen_port = Gtk.Template.Child()
    remote_dest_addr = Gtk.Template.Child()
    remote_dest_port = Gtk.Template.Child()
    # Port forwarding - Dynamic
    dynamic_forward_enabled = Gtk.Template.Child()
    dynamic_listen_addr = Gtk.Template.Child()
    dynamic_listen_port = Gtk.Template.Child()
    ...
    def setup_ui(self):
        ...
        self.local_forward_enabled.connect('notify::active', self.on_port_forward_enabled_changed)
        self.remote_forward_enabled.connect('notify::active', self.on_port_forward_enabled_changed)
        self.dynamic_forward_enabled.connect('notify::active', self.on_port_forward_enabled_changed)
        
        # Populate SSH keys dropdown
        self.populate_ssh_keys()
    
    def populate_ssh_keys(self):
        """Populate SSH key dropdown with available keys"""
        ssh_keys = Gtk.StringList()
        ...
        if os.path.exists(ssh_dir):
            ...
            for key_file in key_files:
                ...
                if os.path.exists(key_path):
                    ...
                    ssh_keys.append(key_file)  # Show only key name, not full path
        
        # Add browse option at the end
        ssh_keys.append('Browse for key file...')
        
        self.keyfile_row.set_model(ssh_keys)
        
        # Set first available key as default, or browse option if no keys found
        if found_keys:
            self.keyfile_row.set_selected(0)  # First key
        else:
            self.keyfile_row.set_selected(0)  # Browse option
        
        # Store the mapping of display names to full paths
        self.key_paths = {}
        for key_file in found_keys:
            self.key_paths[key_file] = os.path.join(ssh_dir, key_file)
        
        # Connect to selection change to handle browse option
        self.keyfile_row.connect('notify::selected', self.on_keyfile_changed)
    
    def on_keyfile_changed(self, combo_row, param):
        """Handle SSH key file selection change"""
        selected_idx = combo_row.get_selected()
        model = combo_row.get_model()
        
        if selected_idx >= 0:
            selected_text = model.get_string(selected_idx)
            
            if selected_text == 'Browse for key file...':
    ...
    def browse_for_key_file(self):
        ...
        file_dialog.set_title('Select SSH Private Key')
        
        # Set initial folder to ~/.ssh
        ssh_dir = os.path.expanduser('~/.ssh')
        if os.path.exists(ssh_dir):
            ssh_folder = Gio.File.new_for_path(ssh_dir)
            file_dialog.set_initial_folder(ssh_folder)
        
        # Create file filter for common key files
        filter_keys = Gtk.FileFilter()
        filter_keys.set_name('SSH Private Keys')
        filter_keys.add_pattern('id_*')
        filter_keys.add_pattern('*_rsa')
        filter_keys.add_pattern('*_ed25519')
        filter_keys.add_pattern('*_ecdsa')
        filter_keys.add_pattern('*_dsa')
        ...
        filters.append(filter_keys)
        filters.append(filter_all)
        file_dialog.set_filters(filters)
        file_dialog.set_default_filter(filter_keys)
        
        # Show dialog
        file_dialog.open(self, None, self.on_key_file_selected)
    ...
    def on_key_file_selected(self, dialog, result):
        ...
        try:
            ...
            if file:
                ...
                # Add the selected file to our dropdown
                model = self.keyfile_row.get_model()
                
                # Remove the "Browse..." option temporarily
                new_model = Gtk.StringList()
                
                # Add existing keys
                for i in range(model.get_n_items() - 1):  # Exclude browse option
                    new_model.append(model.get_string(i))
                
                # Add the new key
                new_model.append(file_name)
                new_model.append('Browse for key file...')  # Re-add browse option
                
                # Update the model and select the new key
                self.keyfile_row.set_model(new_model)
                
                # Store the full path
                self.key_paths[file_name] = file_path
                
                # Select the newly added key
                for i in range(new_model.get_n_items()):
                    if new_model.get_string(i) == file_name:
                        self.keyfile_row.set_selected(i)
                        break
                        
        except Exception as e:
            logger.warning(f"Failed to select key file: {e}")
            # Reset to first option if browse failed
            self.keyfile_row.set_selected(0)
    
    def on_auth_method_changed(self, combo_row, param):
        """Handle authentication method change"""
        selected = combo_row.get_selected()
        
        if selected == 0:  # Key-based
            self.keyfile_row.set_visible(True)
            self.key_passphrase_row.set_visible(True)
            self.password_row.set_visible(False)
        elif selected == 1:  # Password
            self.keyfile_row.set_visible(False)
            self.key_passphrase_row.set_visible(False)
            self.password_row.set_visible(True)
    
    def on_port_forward_enabled_changed(self, switch_row, param):
        """Handle port forwarding enable/disable"""
        # Local port forwarding
        local_enabled = self.local_forward_enabled.get_active()
        self.local_listen_addr.set_sensitive(local_enabled)
        self.local_listen_port.set_sensitive(local_enabled)
        self.local_dest_addr.set_sensitive(local_enabled)
        self.local_dest_port.set_sensitive(local_enabled)
        
        # Remote port forwarding
        remote_enabled = self.remote_forward_enabled.get_active()
        self.remote_listen_addr.set_sensitive(remote_enabled)
        self.remote_listen_port.set_sensitive(remote_enabled)
        self.remote_dest_addr.set_sensitive(remote_enabled)
        self.remote_dest_port.set_sensitive(remote_enabled)
        
        # Dynamic port forwarding
        dynamic_enabled = self.dynamic_forward_enabled.get_active()
        self.dynamic_listen_addr.set_sensitive(dynamic_enabled)
        self.dynamic_listen_port.set_sensitive(dynamic_enabled)
        
        # If any type of forwarding is enabled, disable the others
        if local_enabled:
            self.remote_forward_enabled.set_active(False)
            self.dynamic_forward_enabled.set_active(False)
        elif remote_enabled:
            self.local_forward_enabled.set_active(False)
            self.dynamic_forward_enabled.set_active(False)
    ...
    def load_connection_data(self):
        ...
        if self.connection:
            ...
            self.port_row.set_text(str(self.connection.port or 22))
            self.x11_row.set_active(self.connection.x11_forwarding or False)
            
            # Authentication
            keyfile = self.connection.keyfile or ''
            has_password = hasattr(self.connection, 'password') and self.connection.password
            has_key_passphrase = hasattr(self.connection, 'key_passphrase') and self.connection.key_passphrase
            
            if has_password and not keyfile:
                self.auth_method_row.set_selected(1)  # Password
            else:
                self.auth_method_row.set_selected(0)  # Key-based
            
            if has_key_passphrase:
                self.key_passphrase_row.set_text(self.connection.key_passphrase)
            
            # Set key file selection
            if keyfile:
                # Try to find the key file in the dropdown
                key_name = os.path.basename(keyfile)
                model = self.keyfile_row.get_model()
                found = False
                
                for i in range(model.get_n_items()):
                    display_name = model.get_string(i)
                    if display_name != 'Browse for key file...':
                ...
                if not found:
                    ...
                    for i in range(model.get_n_items() - 1):  # Exclude browse option
                        new_model.append(model.get_string(i))
                    new_model.append(key_name)
                    new_model.append('Browse for key file...')  # Re-add browse option
                    self.keyfile_row.set_model(new_model)
                    self.key_paths[key_name] = keyfile
                    
                    # Select the custom key
                    for i in range(new_model.get_n_items()):
                        if new_model.get_string(i) == key_name:
                            self.keyfile_row.set_selected(i)
                            break
            
            # Local Port Forwarding
            local_forward = self.connection.local_forward
            if local_forward:
                self.local_forward_enabled.set_active(local_forward.get('enabled', False))
                self.local_listen_addr.set_text(local_forward.get('listen_addr', 'localhost'))
                self.local_listen_port.set_text(str(local_forward.get('listen_port', 8080)))
                self.local_dest_addr.set_text(local_forward.get('dest_addr', 'localhost'))
                self.local_dest_port.set_text(str(local_forward.get('dest_port', 80)))
            
            # Remote Port Forwarding
            remote_forward = self.connection.remote_forward
            if remote_forward:
                self.remote_forward_enabled.set_active(remote_forward.get('enabled', False))
                self.remote_listen_addr.set_text(remote_forward.get('listen_addr', 'localhost'))
                self.remote_listen_port.set_text(str(remote_forward.get('listen_port', 8080)))
                self.remote_dest_addr.set_text(remote_forward.get('dest_addr', 'localhost'))
                self.remote_dest_port.set_text(str(remote_forward.get('dest_port', 80)))
            
            # Dynamic Port Forwarding
            dynamic_forward = self.connection.dynamic_forward
            if dynamic_forward:
                self.dynamic_forward_enabled.set_active(dynamic_forward.get('enabled', False))
                self.dynamic_listen_addr.set_text(dynamic_forward.get('listen_addr', 'localhost'))
                self.dynamic_listen_port.set_text(str(dynamic_forward.get('listen_port', 1080)))
            
            # Trigger control states
            self.on_auth_method_changed(self.auth_method_row, None)
            self.on_port_forward_enabled_changed(None, None)
    ...
    def on_save_clicked(self, button):
        ...
        if not host:
            self.show_error('Please enter a host')
            return
        ...
        if auth_method == 0:  # Key-based
            selected_key_idx = self.keyfile_row.get_selected()
            if selected_key_idx >= 0:
                model = self.keyfile_row.get_model()
                selected_key_name = model.get_string(selected_key_idx)
                
                # Don't use browse option as keyfile
                if selected_key_name != 'Browse for key file...':
                    # Get full path from mapping
                    keyfile = self.key_paths.get(selected_key_name, '')
            key_passphrase = self.key_passphrase_row.get_text().strip()
        elif auth_method == 1:  # Password
        ...
        connection_data = {
            ...
            'port_forwarding_enabled': (
                self.local_forward_enabled.get_active() or
                self.remote_forward_enabled.get_active() or
                self.dynamic_forward_enabled.get_active()
            ),
            
            # Local forwarding
            'local_forward': {
                'enabled': self.local_forward_enabled.get_active(),
                'listen_addr': self.local_listen_addr.get_text(),
                'listen_port': int(self.local_listen_port.get_text() or '8080'),
                'dest_addr': self.local_dest_addr.get_text(),
                'dest_port': int(self.local_dest_port.get_text() or '80')
            },
            
            # Remote forwarding
            'remote_forward': {
                'enabled': self.remote_forward_enabled.get_active(),
                'listen_addr': self.remote_listen_addr.get_text(),
                'listen_port': int(self.remote_listen_port.get_text() or '8080'),
                'dest_addr': self.remote_dest_addr.get_text(),
                'dest_port': int(self.remote_dest_port.get_text() or '80')
            },
            
            # Dynamic forwarding
            'dynamic_forward': {
                'enabled': self.dynamic_forward_enabled.get_active(),
                'listen_addr': self.dynamic_listen_addr.get_text(),
                'listen_port': int(self.dynamic_listen_port.get_text() or '1080')
            }
        }
        
        # Set tunnel_type based on which forwarding is enabled
        if connection_data['local_forward']['enabled']:
            connection_data['tunnel_type'] = 'local'
            connection_data['tunnel_port'] = connection_data['local_forward']['listen_port']
        elif connection_data['remote_forward']['enabled']:
            connection_data['tunnel_type'] = 'remote'
            connection_data['tunnel_port'] = connection_data['remote_forward']['listen_port']
        elif connection_data['dynamic_forward']['enabled']:
            connection_data['tunnel_type'] = 'dynamic'
            connection_data['tunnel_port'] = connection_data['dynamic_forward']['listen_port']
        else:
            connection_data['tunnel_type'] = ''
            connection_data['tunnel_port'] = None
    ...
    def show_error(self, message):
        ...
        error_dialog = Adw.MessageDialog.new(self, 'Error', message)
...
class MainWindow(Adw.ApplicationWindow):
    ...
    def setup_signals(self):
        ...
        self.connection_manager.connect('connection-status-changed', self.on_connection_status_changed)
    ...
    def on_tab_close(self, tab_view, page):
        ...
        # Find corresponding connection and disconnect
    ...
    def on_quit_confirmation_response(self, dialog, response):
        ...
        if response == 'quit':
            self.cleanup_and_close()
            # Always quit the application (this works for both window close and app quit)
            app = self.get_application()
            if app:
    ...
    def cleanup_and_close(self):
        ...
        try:
            ...
            try:
                ...
                sidebar_width = 250  # default width
            self.config.save_window_geometry(width, height, sidebar_width)
            
            logger.info("Application cleanup completed")
            
        except Exception as e:
            logger.error(f"Error during window close cleanup: {e}")
